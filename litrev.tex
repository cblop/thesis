...
The \emph{Mimesis} architecture is designed to integrate into the \emph{Unreal
  Tournament} game engine, and has five components:

\begin{enumerate}
\item The \textbf{mimesis controller} (MC), to which all components connect and send
  messages.
\item The \textbf{story planner}, which handles plan requests from the game engine.
\item The \textbf{discourse planner}, which handles the presentation of the game
  by controlling non-character media such as cameras and music.
\item The \textbf{execution manager}, which drives the story's action forward.
  It builds a directed acyclic graph whose nodes represent actions from the plan
  and edges are temporal constraints that order the actions.
\item The \textbf{MWorld}, which includes three elements: the game engine,
  communication-coordinating code, and \emph{action classes}, which specify the
  behaviors of each action operator in the story and discourse planners.
\end{enumerate}

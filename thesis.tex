...
story planners~\citep{young2004architecture}, combine the use of narrative
...
``Morphology'' or Lehnert's ``Plot Units'' lack any kind of mechanism through
which an author may create abstractions for stories, by creating a new story
``component'' as a combination of existing ``components''. The other shortcoming
is with the goal-oriented authoring style expected by story planners, where
non-programmer story authors are required to author their stories in terms of
``goals'' that the narrative must meet.


This thesis offers two main contributions: the use of a normative framework to
regulate the behaviours of a multi-agent system so that their decisions are
guided by a narrative description, and the definition of this narrative
description in terms of \emph{story tropes}, which act as a formalism for the
expression of composable, abstractable story components. A trope is a recurring
theme or motif in narrative, which abstracts
certain sequence of events. After a thorough examination of existing formalisms
for narrative and systems for interactive narrative generation, we show how tropes allow for the flexible description and abstraction of story components, a capability that existing systems lack.


These techniques are demonstrated through the development of two tools for
...
tropes in \emph{TropICAL}. \emph{TropICAL} has a controlled natural language
syntax, and compiles to the \emph{InstAL} language for describing institutions
(sets of norms which regulate the behaviour of agents), which itself compiles to
AnsProlog, an Answer Set Programming (ASP) language. Once compiled to AnsProlog
code, the system is able to use an Anwser Set solver to generate all the
different possible paths through the story for any given set of trope, and to
ensure that a combination of events in a story are valid for any set of tropes.

We evaluate the suitability of these tools for interactive story construction through a thematic analysis of story authors' completion of story-authoring tasks using the language. The participants complete tasks in which they have to describe stories with different degrees of complexity, finally requiring them to reuse existing tropes in their own trope abstractions. The thematic analysis identifies and examines the themes and patterns that emerge from the story authors' use of the tool.
\end{abstract}
...
structural control of the branching narratives, showing control of the narrative
...
depending on how quickly you can piece together clues. This is the new frontier that we are exploring with this research: the as-yet uncharted domain of \emph{interactive} storytelling.
...
to enable a whole new level of narrative immersion.\mnote{Toned this down a
  little bit}
...
\mnote{Removed the ``interactive narrative for education'' section. Will mention
the Bishop's Palace work in summary at the end}
...
way that the character agents respond to these interactions.\mnote{Added bit
...
example, \emph{Then the ``Quest'' trope happens}).\mnote{added how requirements
  are met}
...
\begin{compactenum}
\item A \textbf{Literature Review} (chapter~\ref{cha:literature-review}).
  Because we are developing a formalism with which to describe narrative
  components, the literature review first covers narrative research
  ...
  generation systems from computer science (section~\ref{sec:implementations}).
\item The use of \textbf{Institutions as Story Worlds} is described in
  chapter~\ref{cha:institutions}, exploring the theory and research behind
  ...
  from the previous section. This chapter is a development of ``Governing
  Narrative Events with Institutional Norms'', which is listed as
  the \textbf{[CMN 2015]} conference paper in the preamble.
\item Using \textbf{Tropes as Story Components},
  chapter~\ref{cha:tropes},
  examines the use of \emph{tropes} as story components, and why they improve
  upon existing narrative formalisms. This is linked to the work in
  chapter~\ref{cha:institutions} by describing how tropes can be used as a
  ``user interface'' through which story authors would be able to simply
  describe the social institutions that govern story characters' behaviours.
  Work from this chapter appears in a legal
  context in~\cite{thompson2016describing}.
\item In order to provide context for how social institutions are useful for
  governing character agents, chapter~\ref{cha:agents} explains how
  \textbf{Intelligent Agents} may be used to simulate characters in a story
  (chapter~\ref{cha:agents}), describing work done as part of the \textbf{[AISB
    2015]} paper.
\item Following on from the requirements derived from
  chapters~\ref{cha:institutions} to~\ref{cha:agents},
  chapter~\ref{cha:tropical} describes the design and implementation of our \textbf{TropICAL} programming
  language for tropes, along with examples of its use to build several stories.
\item Chapter~\ref{cha:storybuilder} develops the concept behind
  \emph{TropICAL} further with the implementation of the ``StoryBuilder'' tool,
  ...
  narrative. This chapter also contains a qualitative evaluation of the tool
  through a thematic analysis of eight participants building their own stories
  with the tool.
\item The final chapter, \textbf{Conclusions and Future Work}, looks back at the
  research done, evaluating its potential impact and discussing possible future work.
\end{compactenum}
...
\begin{quote}
  ...
  Twine's user inteface uses the analogy of notes on a pinboard connected with
  ...
  Next, Bob want to get more ambitious by abstracting away different parts of

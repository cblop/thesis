...
Exclusion logic is designed to express the fact that a variable can only have
one value. This saves the author from having to remember to change extra facts.
For example, if the ``Brown'' character changes location from the dining room to
the kitchen, \emph{brown.in!dining\_room} is terminated when \emph{brown.in!kitchen} holds.

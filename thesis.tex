...
We want them to appear in our story as well, but one after another, in sequence.
We will achieve this by using one as a subtrope of another. The two scenes that
we will describe are the ``Babysitting scene'', where Judy leaves Punch with the
...
that its events can occur at any point. This is described further in
section~\ref{sec:combining-tropes} below.

\begin{lstlisting}[label={list:audience-participation},caption={The ``Audience participation'' trope}]
...
Listing~\ref{lst:list:babysitting-trope} shows a trope to describe the
...
\begin{lstlisting}[label={list:babysitting-trope},caption={The ``Babysitting'' trope}]
...
Listing~\ref{lst:list:sausages-trope} shows the TropICAL definition of the
...
\begin{lstlisting}[label={list:sausages-trope},caption={The ``Crocodile Sausages'' trope}]
...
\subsubsection{Combining Tropes Together}
...
``Babysitting'' scene are combined with the ``Audience Participation'' trope. If
both tropes are described in TropICAL and compiled to InstAL, they describe
social norms that regulate the actions of character agents. When both of these
tropes are given as inputs to an answer set solver, then we are able to generate
all of the possible permutations of the possible events of both of these tropes.
...
evaluated qualitatively through a thematic analysis (described by~\citet{clarke2014thematic}).
...
create tropes for a story of their own.
...
\begin{itemize}
  \item Tropes can have a maximum of five events in a sequence (not including branches)
  \item Subtropes must go at the end of a trope
  \item Branches cannot be extended beyond one event
  \item Branches cannot be cut off (by writing ``Then the story ends''), and so
    always merge back into the main story
  \item The tropes must be event-based. This means that updating the state of
    ...
    Apple'' or ``The Apple rests in the Barn''.
...
software. The full transcripts are listed in
Appendix~\ref{appendix:transcripts}.

\subsection{Identified Themes}
% ATODO intro: 1
% TODO list themes: 4
We identify eighteen different themes from the participant transcripts listed in
...
version of StoryBuilder. The user was able to select named characters to fulfil
...
for the participants while visualising their tropes. Some users would write
events where one character is killed, such as ``The Villain kills the Hero'',
and expect all subsequent branches featuring the Hero to disappear after that
...
As explained above, the \emph{clingo} answer-set solver that StoryBuilder uses
...
answer sets are produced by the solver, the error message ``Compilation Error''

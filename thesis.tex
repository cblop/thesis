...
tropes'' are created in section~\ref{sec:subtrope-code}, the intermediate data structure used between parsing and
...
\begin{compactitem}
  \item A way to describe the story such that the agents are told what to do
    next, but are able to break when necessary.
\end{compactitem}
...
certain actions, so section~\label{sec:obl-code} shows how an author may
...
completely optional. If the name of an entity begins with ``The'', the parser
...
character goes to a place called \emph{The Land of Adventure}.

Typically, tropes tend to consist
of a sequence of several events, so this trope could be extended further:
...
stemmed using \emph{WordNet}~\ref{miller1995wordnet} so that, for example,
...
\subsubsection{Deadline Events}

\subsubsection{Violation Events and Consequences}

\subsection{Branching Events}
\label{sec:branch-code}
A trope author may want to describe alternative possibilities in their story,
...
\emph{Subtropes} are simply previously-written tropes that are embedded inside a
...
\label{sec:t-grammar}
% TODO intro: 1
This section describes the grammar of our TropICAL language formally using
\emph{Extended Backus-Naur Form} notation.




% TODO instaparse description: 1
We used the Clojure programming language~\citep{clojure} with the Instaparse
...
<the> = <'The ' | 'the '>
<word> = #'[0-9a-zA-Z\\-\\_\\']*'
\end{lstlisting}\mnote{See above}

\section{Parsing Examples}
\label{sec:parse-tree}
...
This section shows visualisations of the parse tree that results from the trope
...
This section first describes how specific language features translate to
InstAL, then goes on to list the InstAL output of full trope translation.
...
institutions.\mnote{More detail needed here}
The two institutions linked are referred to as the
...
Potassco project's Clingo~\cite{gebser2011potassco} solver to formally verify
...
\begin{lstlisting}[label={lst:constraints}, caption={Constraint rules to remove
invalid and irrelevant events}, escapechar=|]
...
The answer set solver produces six traces for this trope.
...
Combining the two tropes together produces 124 answer sets. Again, the reason
...
that would seem like a more suitable place to finish the story, rather than
...
These policies can be expressed in terms of social norms:

\begin{itemize}
\item \textbf{The Warranty}: The seller sells an item to a buyer. Then,
  if the item is defective, the buyer \emph{may} return it.
\item \textbf{The Lease}: A lessor leases an item to a lessee. The lessee
  \emph{must} pay the rent \emph{before} the due date, \emph{otherwise} the
  lessor \emph{may} cancel the lease.
  \item \textbf{The Deposit}: The lessee gives a sum of money to the lessor.
    When the lease ends, if the leased item is in good condition, then the
    lessor \emph{must} return the deposit to the lessee.
\end{itemize}

Once legal policies are expressed in this way, they can be translated into
TropICAL code:

\begin{figure}[!t]
\begin{lstlisting}[belowskip=-0.5em]
...
\end{lstlisting}
 % \caption{The ``Lease'' policy in TropICAL\label{lst:lease}}
% \medskip
% \begin{lstlisting}[float=t!,caption={The ``Evil Empire'' trope in TropICAL},label=lst:evil]
\begin{lstlisting}[belowskip=-0.5em]
...
\end{lstlisting}%
% \caption{The ``Sublease'' policy in TropICAL\label{lst:sublease}}
% \medskip
% \begin{lstlisting}[float=t!,caption={The ``Quest'' trope in TropICAL},label=lst:quest]
\begin{lstlisting}[belowskip=-0.5em]
...
\end{lstlisting}
% \caption{The ``Sublease Permission'' policy in TropICAL\label{lst:sublease-permission}}
% \medskip
\begin{lstlisting}[belowskip=-0.5em]
...
\end{lstlisting}
\caption{Example policies in TropICAL\label{lst:policies}}
\end{figure}

The use of TropICAL to describe legal policies would be useful for lawyers that
want to validate or generate arguments from any given set of policies. This would be invaluable as a tool
